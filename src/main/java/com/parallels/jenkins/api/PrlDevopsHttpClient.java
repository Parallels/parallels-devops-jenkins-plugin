        ...
        while (true) {
            ...
            switch (state) {
                case "running":
                    ...
                    // The Parallels guest agent may still be initialising even after the VM
                    // reports "running", so we retry until success or timeout.
                    while (true) {
                        try {
                            ...
                            if (probe.getExitCode() == 0) {
                                return status; // execute API confirmed working
                            }
